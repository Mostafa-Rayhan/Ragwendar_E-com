import React from 'react'

import returnImg from "../assets/img/easy_return.png"
import logo from "../assets/img/logo.png" 

const Footer = () => {
  return (
    <footer className="footerdiv py-5">
    <div className="container">
      <div className="row text-center">
        <div className="col">
          <img src={returnImg} height="30" alt="" /> 
          <h6 className="m-0 fw-bold">EASY RETURN</h6>
          <span>30 Day Return Policy</span>
        </div>
        <div className="col">
          <img src="assets/img/free-delivery.png" height="30" alt="" />
          <h6 className="m-0 fw-bold">FREE DELIVERY</h6>
          <span>Payment Online</span>
        </div>
        <div className="col">
          <img src="assets/img/cod.png" height="30" alt="" />
          <h6 className="m-0 fw-bold">COD AVAILABLE</h6>
          <span>@ Rs. 49 Per Order</span>
        </div>
        <div className="col">
          <img src="assets/img/Screenshot_118.png" height="30" alt="" />
       
          <h6 className="m-0 fw-bold">SECURE PAYMENTS</h6>
          <span>100% Assurance</span>
        </div>
      </div>
      <hr />
      <div className="row pt-4">
        <div className="col-xs-12">
          <a href="index.html" className="navbar-brand brand-style"><img src={logo} alt="e-Pharmacy"
              height="50" srcset="" className="d-inline-block align-text-top logo-navbar" /></a>
        </div>
      </div>
      <div className="row footer p-4">
        <div className="col-sm-6 col-md-3">
          <h5>Customer Service</h5>
          <ul>
            <li><a href="">track my order</a></li>
            <li><a href="">Help</a></li>
            <li><a href="">returns</a></li>
            <li><a href="">delivery info</a></li>
            <li><a href="">return order</a></li>
            <li><a href="">payment method</a></li>
          </ul>
        </div>
        <div className="col-sm-6 col-md-3">
          <h5>Company Info</h5>
          <ul>
            <li><a href="">about</a></li>
            <li><a href="">size guide</a></li>
            <li><a href="">app</a></li>
            <li><a href="">gift cards</a></li>
            <li><a href="">become an affiliate</a></li>
            <li><a href="">become a partner</a></li>
            <li><a href="">Discount & promo codes</a></li>
            <li><a href="">blog</a></li>
          </ul>
        </div>
        <div className="col-sm-6 col-md-2">
          <h5>Help & Support</h5>
          <ul>
            <li><a href="">contact us</a></li>
            <li><a href="">refund policy</a></li>
            <li><a href="">privacy policy</a></li>
            <li><a href="">terms & conditions</a></li>
            <li><a href="">faq's</a></li>
            <li><a href="">sitemap</a></li>
          </ul>
        </div>
        <div className="col-sm-6 col-md-4">
          <h5>Join us for get updates</h5>
          <ul>
            <li>
              <div className="d-flex">
                <input type="text" placeholder="Enter Your Email" />
                <button className="s-btn">Subscribe</button>
              </div>
            </li>
          </ul>
          <h5 className="mt-2">Connect with us</h5>
          <div className="social-icons fs-2 d-flex">
            <a className="me-1" href=""><i className='bx bxl-facebook'></i></a>
            <a className="me-1" href=""><i className='bx bxl-instagram'></i></a>
            <a className="me-1" href=""><i className='bx bxl-twitter'></i></a>
            <a className="me-1" href=""><i className='bx bxl-youtube'></i></a>
            <a className="me-1" href=""><i className='bx bxl-pinterest-alt'></i></a>
          </div>
          <h5 className="mt-2">Download the app</h5>
          <div className="social-icons fs-2 d-flex">
            <a className="me-3" href=""><img height="30" src="assets/img/gplay.png" alt="" /></a>
            <a className="me-3" href=""><img height="30" src="assets/img/appStore.png" alt="" /></a>
          </div>
        </div>
      </div>
      <div className="row">
        <div className="col-sm-12 col-md-6">
          <div tabindex="0" role="contentinfo" className="bank-img p-4 text-md-end"
            aria-label="We accept PayPal, Visa, Mastercard, Maestro, and more.">
            100% SECURE PAYMENT &nbsp;
            <img aria-hidden="true" tabindex="-1"
              data-src="//img.ltwebstatic.com/images3_pi/2021/03/09/161528368123dd7a35ad8708b0dfc74b3630526891.png"
              className="c-img-con-m j-verlok-lazy loaded"
              data-lazy-src="//img.ltwebstatic.com/images3_pi/2021/03/09/161528368123dd7a35ad8708b0dfc74b3630526891.png"
              src="//img.ltwebstatic.com/images3_pi/2021/03/09/161528368123dd7a35ad8708b0dfc74b3630526891.png"
              data-was-processed="true" />
            <img aria-hidden="true" tabindex="-1"
              data-src="//img.ltwebstatic.com/images2_pi/2018/06/06/15282732983375743706.png"
              className="c-img-con-m j-verlok-lazy loaded"
              data-lazy-src="//img.ltwebstatic.com/images2_pi/2018/06/06/15282732983375743706.png"
              src="//img.ltwebstatic.com/images2_pi/2018/06/06/15282732983375743706.png" data-was-processed="true" />

            <img aria-hidden="true" tabindex="-1"
              data-src="//img.ltwebstatic.com/images2_pi/2018/06/06/1528273151799711689.png"
              className="c-img-con-m j-verlok-lazy loaded"
              data-lazy-src="//img.ltwebstatic.com/images2_pi/2018/06/06/1528273151799711689.png"
              src="//img.ltwebstatic.com/images2_pi/2018/06/06/1528273151799711689.png" data-was-processed="true" />
            <img aria-hidden="true" tabindex="-1"
              data-src="//img.ltwebstatic.com/images3_pi/2020/09/23/16008293560df34023305e4c8617d6886e33b59531.png"
              className="c-img-con-m j-verlok-lazy loaded"
              data-lazy-src="//img.ltwebstatic.com/images3_pi/2020/09/23/16008293560df34023305e4c8617d6886e33b59531.png"
              src="//img.ltwebstatic.com/images3_pi/2020/09/23/16008293560df34023305e4c8617d6886e33b59531.png"
              data-was-processed="true" />
            <img aria-hidden="true" tabindex="-1"
              data-src="//img.ltwebstatic.com/images2_pi/2018/06/06/15282733431754785346.png"
              className="c-img-con-m j-verlok-lazy loaded"
              data-lazy-src="//img.ltwebstatic.com/images2_pi/2018/06/06/15282733431754785346.png"
              src="//img.ltwebstatic.com/images2_pi/2018/06/06/15282733431754785346.png" data-was-processed="true" />
            <img aria-hidden="true" tabindex="-1"
              data-src="//img.ltwebstatic.com/images3_pi/2021/01/15/1610701410b3781f00695b77b833e6b6a5e38331a3.png"
              className="c-img-con-m j-verlok-lazy loaded"
              data-lazy-src="//img.ltwebstatic.com/images3_pi/2021/01/15/1610701410b3781f00695b77b833e6b6a5e38331a3.png"
              src="//img.ltwebstatic.com/images3_pi/2021/01/15/1610701410b3781f00695b77b833e6b6a5e38331a3.png"
              data-was-processed="true" />
          </div>
        </div>
        <div className="col-sm-12 col-md-6">
          <div tabindex="0" role="contentinfo" className="bank-img p-4"
            aria-label="We accept PayPal, Visa, Mastercard, Maestro, and more.">
            SHIPPING PARTNER :&nbsp;
            <img aria-hidden="true" tabindex="-1"
              data-src="//img.ltwebstatic.com/images3_pi/2021/03/09/161528368123dd7a35ad8708b0dfc74b3630526891.png"
              className="c-img-con-m j-verlok-lazy loaded"
              data-lazy-src="//img.ltwebstatic.com/images3_pi/2021/03/09/161528368123dd7a35ad8708b0dfc74b3630526891.png"
              src="//img.ltwebstatic.com/images3_pi/2021/03/09/161528368123dd7a35ad8708b0dfc74b3630526891.png"
              data-was-processed="true" />
            <img aria-hidden="true" tabindex="-1"
              data-src="//img.ltwebstatic.com/images2_pi/2018/06/06/15282732983375743706.png"
              className="c-img-con-m j-verlok-lazy loaded"
              data-lazy-src="//img.ltwebstatic.com/images2_pi/2018/06/06/15282732983375743706.png"
              src="//img.ltwebstatic.com/images2_pi/2018/06/06/15282732983375743706.png" data-was-processed="true" />

            <img aria-hidden="true" tabindex="-1"
              data-src="//img.ltwebstatic.com/images2_pi/2018/06/06/1528273151799711689.png"
              className="c-img-con-m j-verlok-lazy loaded"
              data-lazy-src="//img.ltwebstatic.com/images2_pi/2018/06/06/1528273151799711689.png"
              src="//img.ltwebstatic.com/images2_pi/2018/06/06/1528273151799711689.png" data-was-processed="true" />
            <img aria-hidden="true" tabindex="-1"
              data-src="//img.ltwebstatic.com/images3_pi/2020/09/23/16008293560df34023305e4c8617d6886e33b59531.png"
              className="c-img-con-m j-verlok-lazy loaded"
              data-lazy-src="//img.ltwebstatic.com/images3_pi/2020/09/23/16008293560df34023305e4c8617d6886e33b59531.png"
              src="//img.ltwebstatic.com/images3_pi/2020/09/23/16008293560df34023305e4c8617d6886e33b59531.png"
              data-was-processed="true" />
            <img aria-hidden="true" tabindex="-1"
              data-src="//img.ltwebstatic.com/images2_pi/2018/06/06/15282733431754785346.png"
              className="c-img-con-m j-verlok-lazy loaded"
              data-lazy-src="//img.ltwebstatic.com/images2_pi/2018/06/06/15282733431754785346.png"
              src="//img.ltwebstatic.com/images2_pi/2018/06/06/15282733431754785346.png" data-was-processed="true" />
            <img aria-hidden="true" tabindex="-1"
              data-src="//img.ltwebstatic.com/images3_pi/2021/01/15/1610701410b3781f00695b77b833e6b6a5e38331a3.png"
              className="c-img-con-m j-verlok-lazy loaded"
              data-lazy-src="//img.ltwebstatic.com/images3_pi/2021/01/15/1610701410b3781f00695b77b833e6b6a5e38331a3.png"
              src="//img.ltwebstatic.com/images3_pi/2021/01/15/1610701410b3781f00695b77b833e6b6a5e38331a3.png"
              data-was-processed="true" />
          </div>
        </div>
      </div>
      <div className="container-fluid">
        <div className="row p-md-4">
          <div className="col-12">

            <button className="btn btn-outline-none collapseBtnText toggleBtn1" type="button" onclick="btn1()"
              aria-expanded="false">
              Who We Are
            </button>
          </div>
          <div className="col-10 collapse multi-collapse collapseText px-5 text1 " id="collapse1">
            Lorem ipsum dolor sit, amet consectetur adipisicing elit. Ipsa id quis assumenda similique odio dicta eius
            aliquid, consequuntur, et temporibus ducimus maxime velit magnam! Repellendus blanditiis at aut autem nulla.
          </div>
          <span className="border-bottom"></span>
          <div className="col-12">
            <button className="btn btn-outline-none collapseBtnText toggleBtn2" type="button" onclick="btn2()"
              aria-expanded="false">
              Why Chose Us
            </button>
          </div>
          <div className="col-10 collapse multi-collapse collapseText text2 px-5" id="collapse2">
            Lorem ipsum dolor sit, amet consectetur adipisicing elit. Ipsa id quis assumenda similique odio dicta eius
            aliquid, consequuntur, et temporibus ducimus maxime velit magnam! Repellendus blanditiis at aut autem nulla.
          </div>
          <span className="border-bottom"></span>
          <div className="col-12">
            <button className="btn btn-outline-none collapseBtnText toggleBtn3" type="button" onclick="btn3()"
              aria-expanded="false">
              Navigation Link
            </button>
          </div>
          <div className="col-10 collapse multi-collapse collapseText px-5 text3 " id="collapse3">
            Lorem ipsum dolor sit, amet consectetur adipisicing elit. Ipsa id quis assumenda similique odio dicta eius
            aliquid, consequuntur, et temporibus ducimus maxime velit magnam! Repellendus blanditiis at aut autem nulla.
          </div>
          <span className="border-bottom"></span>
        </div>
        <h5 className="fs-6 text-center">&copy;2022 All Rights Reserved</h5>
      </div>
    </div>
  </footer>
  )
}

export default Footer;